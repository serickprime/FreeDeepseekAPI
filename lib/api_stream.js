...
const { STREAM_PROTOCOL_MARKERS } = require('./tool_containment');
...
function createProtocolStream(res, { kind, id, model, created, bufferForTools = false, quarantineToolProtocol = false }) {
  res.writeHead(200, {
  ...
  function longestMarkerPrefixSuffix(value) {
    ...
    for (const marker of STREAM_PROTOCOL_MARKERS) {
      ...
      for (let size = limit; size > length; size -= 1) {
  ...
  function quarantinedDelta(fragment) {
    const mode = fragment.reasoning ? 'reasoning' : 'content';
    const value = fragment.reasoning || fragment.content || '';
    if (quarantineTriggered) {
      quarantineBuffer.push({ [mode]: value });
      return;
    }
    if (quarantineMode !== null && quarantineMode !== mode && quarantineTail) {
      emitDelta({ [quarantineMode]: quarantineTail });
      quarantineTail = '';
    }
    quarantineMode = mode;
    const candidate = quarantineTail + value;
    let markerIndex = -1;
    for (const marker of STREAM_PROTOCOL_MARKERS) {
    ...
    if (markerIndex >= 0) {
      if (markerIndex > 0) emitDelta({ [mode]: candidate.slice(0, markerIndex) });
      quarantineTriggered = true;
      quarantineBuffer.push({ [mode]: candidate.slice(markerIndex) });
      quarantineTail = '';
      return;
    }
    const suffixLength = longestMarkerPrefixSuffix(candidate);
    const safeLength = candidate.length - suffixLength;
    if (safeLength > 0) emitDelta({ [mode]: candidate.slice(0, safeLength) });
    quarantineTail = candidate.slice(safeLength);
  }

  function delta(fragment) {
    if (finished || bufferForTools) return;
    sawDelta = true;
    if (quarantineToolProtocol) return quarantinedDelta(fragment);
    emitDelta(fragment);
  }

  function finish({ output, toolCall, finalResponse, protocolContained = false }) {
    if (finished) return;
    finished = true;
    ...
    } else if (quarantineToolProtocol && !toolCall) {
      if (protocolContained) {
        quarantineTail = '';
        quarantineBuffer.length = 0;
        if (output.reasoning) deltaBuffered({ reasoning: output.reasoning });
        if (output.content) deltaBuffered({ content: output.content });
      } else if (quarantineTriggered) {
        for (const fragment of quarantineBuffer) deltaBuffered(fragment);
      } else if (quarantineTail) {
        deltaBuffered({ [quarantineMode || 'content']: quarantineTail });
      } else if (!sawDelta) {
        if (output.reasoning) deltaBuffered({ reasoning: output.reasoning });
        if (output.content) deltaBuffered({ content: output.content });
      }
