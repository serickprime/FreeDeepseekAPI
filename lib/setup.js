...
function createSetupController({ root = path.resolve(__dirname, '..'), launchTerminal = visiblePowerShell, hasCommand = commandExists } = {}) {
  ...
  async function action(name, options = {}) {
    ...
    if (name === 'claude') {
      ...
      const pid = await launchTerminal(root, command);
      return { ok: true, pid, model: modelName, message: `Claude Code запущен с ${modelName}.` };
    }
    if (name === 'opencode') {
      if (!status().agents.opencode) return { ok: false, message: 'OpenCode не найден. Установите его и обновите страницу.' };
      const pid = await launchTerminal(root, `opencode.cmd --model deepseek-web/${modelName}`);
      return { ok: true, pid, model: modelName, message: `OpenCode запущен с ${modelName}.` };
    }
...
module.exports = { createSetupController, readAuthStatus, visiblePowerShell };
