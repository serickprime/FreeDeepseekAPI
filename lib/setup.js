...
const { safeError } = require('./security');

const DOCTOR_PROCESS_TIMEOUT_MS = 200_000;
...
function readAuthStatus(root) {
  ...
  try {
    ...
    return { present: true, valid: hasValue(auth.token) && hasValue(auth.cookie) };
  } catch {
...
function runDoctor(root) {
  return new Promise(resolve => {
    execFile(process.execPath, [path.join(root, 'scripts', 'doctor.js')], {
      cwd: root,
      timeout: DOCTOR_PROCESS_TIMEOUT_MS,
      windowsHide: true,
      maxBuffer: 64 * 1024,
    }, (error, stdout, stderr) => {
      const output = safeError(`${stdout || ''}${stderr || ''}`)
        .trim()
        .slice(0, 4000);
...
function chooseWindowsFolder(initialDirectory) {
  ...
  const script = [
    ...
    '$owner.Close()',
    'if ($result -eq [System.Windows.Forms.DialogResult]::OK) { [Console]::OutputEncoding = [System.Text.UTF8Encoding]::new(); [Console]::Write($dialog.SelectedPath) }',
  ].join('; ');
...
function existingDirectory(value, fallback) {
  try {
    ...
    const candidate = requested || fallbackValue;
...
function windowsDirectoryRoots(fileSystem = fs) {
  const roots = [];
  for (let code = 65; code <= 90; code += 1) {
    const drive = `${String.fromCharCode(code)}:\\`;
    try {
      if (fileSystem.statSync(drive).isDirectory()) roots.push(drive);
    } catch { /* unavailable, disconnected, or inaccessible drive */ }
  }
  return roots;
}

function listDirectories(value, fallback) {
  const directory = existingDirectory(value, fallback);
  if (!directory) return null;
  try {
    const entries = fs.readdirSync(directory, { withFileTypes: true })
      .filter(entry => entry.isDirectory())
      .sort((left, right) => left.name.localeCompare(right.name, 'ru', { numeric: true, sensitivity: 'base' }));
    const parsedRoot = path.parse(directory).root;
    const roots = process.platform === 'win32' ? windowsDirectoryRoots() : [parsedRoot];
    if (!roots.some(root => root.toLowerCase() === parsedRoot.toLowerCase())) roots.unshift(parsedRoot);
    return {
      currentPath: directory,
      parentPath: path.dirname(directory) === directory ? null : path.dirname(directory),
      roots,
      directories: entries.slice(0, MAX_DIRECTORY_ENTRIES).map(entry => ({
        name: entry.name,
...
function createSetupController({
  ...
  async function action(name, options = {}) {
    ...
    if (name === 'validate-folder') {
      const requested = typeof options.workingDirectory === 'string' ? options.workingDirectory.trim() : '';
      if (!requested) return { ok: false, message: 'Выберите рабочую папку перед запуском.' };
      const directory = existingDirectory(requested);
      return directory
        ? { ok: true, path: directory }
        : { ok: false, message: 'Папка не существует, не является каталогом или недоступна.' };
    }
    if (name === 'browse-folders') {
      const requested = typeof options.workingDirectory === 'string' ? options.workingDirectory.trim() : '';
      const listing = listDirectories(requested, requested ? undefined : root);
      return listing
        ? { ok: true, ...listing }
        : { ok: false, message: 'Не удалось открыть папку. Проверьте путь и права доступа.' };
    }
    if (name === 'choose-folder') {
      ...
      const selectedDirectory = existingDirectory(selected);
    ...
    const workingDirectory = existingDirectory(options.workingDirectory);
    if ((name === 'claude' || name === 'opencode') && !workingDirectory) {
      const empty = typeof options.workingDirectory !== 'string' || !options.workingDirectory.trim();
      return { ok: false, message: empty
        ? 'Выберите рабочую папку перед запуском.'
        : 'Папка проекта не существует, не является каталогом или недоступна.' };
    }
    if (name === 'claude') {
