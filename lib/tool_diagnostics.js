...
const TOOL_RETRY_REASONS = new Set(['none', 'reasoning_only', 'code_fence', 'prefixed_tool', 'repeated_tool']);
...
function toolResultErrorCount(body, protocol) {
  if (protocol !== 'anthropic') return 0;
  let count = 0;
  for (const message of Array.isArray(body?.messages) ? body.messages : []) {
    for (const block of Array.isArray(message?.content) ? message.content : []) {
      if (block?.type === 'tool_result' && block.is_error === true) count += 1;
    }
  }
  return count;
}
...
function createToolDiagnostics({
  ...
  function request({
    ...
    emit({
      ...
      tool_result_error_count: toolResultErrorCount(body, protocol),
    });
    ...
    return {
      ...
      response({ strictToolCallDetected, selectedToolName, reasoningNonempty, contentNonempty, reasoningRetryAttempted, fencedToolRetryAttempted, prefixedToolRetryAttempted, repeatedToolRetryAttempted, toolRetryReason, toolParseResult, outcome } = {}) {
        if (finished) return;
        finished = true;
        emit({
          event: 'tool_response',
