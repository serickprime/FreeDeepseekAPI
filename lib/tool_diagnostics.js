...
function createToolDiagnostics({
  ...
  function request({
    ...
    return {
      ...
      response({ strictToolCallDetected, reasoningNonempty, contentNonempty, reasoningRetryAttempted, fencedToolRetryAttempted, prefixedToolRetryAttempted, repeatedToolRetryAttempted, toolRetryReason, toolParseResult, outcome } = {}) {
        if (finished) return;
        finished = true;
        emit({
          event: 'tool_response',
