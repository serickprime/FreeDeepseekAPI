...
const TOOL_RETRY_REASONS = new Set(['none', 'reasoning_only', 'code_fence', 'repeated_tool']);
...
function createToolDiagnostics({
  ...
  function request({
    ...
    return {
      ...
      response({ strictToolCallDetected, reasoningNonempty, contentNonempty, reasoningRetryAttempted, fencedToolRetryAttempted, repeatedToolRetryAttempted, toolRetryReason, toolParseResult, outcome } = {}) {
        if (finished) return;
        finished = true;
        emit({
          event: 'tool_response',
