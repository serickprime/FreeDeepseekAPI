...
function safeError(error) {
  let message;
  try { message = typeof error?.message === 'string' ? error.message : String(error || 'Internal error'); }
  catch { return 'Internal error'; }
  return message
    .replace(/\b(?:https?|file):\/\/[^\s,;"'}\]]+/ig, '[REDACTED_URL]')
    .replace(/\\\\[^\r\n]+/g, '[REDACTED_PATH]')
    .replace(/\b[A-Za-z]:[\\/][^\r\n]+/g, '[REDACTED_PATH]')
    .replace(/(^|[\s"'(])\/(?!\/)[^\r\n]+/g, '$1[REDACTED_PATH]')
    .replace(/(Bearer\s+)[^\s,;"'}\]]+/ig, '$1[REDACTED]')
    .replace(/\b(cookie|authorization)\b\s*[:=]\s*[^\r\n]+/ig, '$1=[REDACTED]')
    .replace(/\b([a-z_]*token)\b\s*[:=]\s*(?:"[^"]*"|'[^']*'|[^\s,;}\]]+)/ig, '$1=[REDACTED]');
}
function logSafeError(error, logger = console.error) {
  const message = safeError(error);
  try { if (typeof logger === 'function') logger(`[deepseek-bridge] request error: ${message}`); }
  catch {}
  return message;
}
...
module.exports = { isLoopback, isLocalOrigin, safeError, logSafeError, cors, authorized, assertConfig };
